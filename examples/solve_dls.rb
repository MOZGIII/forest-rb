...
if $0 == __FILE__
  ...
  searcher = Pathfinder::DepthLimitedSearch.new(context)

  # Save map in our format
  MapFormat::Json.save(map, "map.json")

  root_node = Pathfinder::DepthLimitedSearch.build_root_node(SpawnAction.new, start_state)
  length = 10 ** 2
  solution = searcher.search(root_node, length)
  unless solution
    STDERR.puts "Solution not found!"
    STDERR.puts "No route from #{start_state.position.to_a} to #{target_state.position.to_a} with #{length} depth limit"
    exit 1
  end
