...
pathfinder_class = Pathfinder::IterativeDeepeningSearch
searcher = pathfinder_class.new(context)

# Save map in our format
MapFormat::Json.save(map, "map.json")

root_node = pathfinder_class.build_root_node(SpawnAction.new, start_state)
solution = searcher.search(root_node)
unless solution
